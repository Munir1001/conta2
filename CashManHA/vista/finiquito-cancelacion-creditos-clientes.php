<?php
    ...
    class PDF extends FPDF{
        ...
        function Header(){
            $this->Image('../vista/images/modelo-informes/cabecera-simple.jpeg',10,9,190);
        }
    ...
    $pdf->Ln(50);
    $pdf->MultiCell(190,5,utf8_decode("Ecuador, ".utf8_encode(strftime("%A %d de %B de %Y")).""),'','R');
    $pdf->Ln(5);
    $pdf->Ln(5);
    ...
    $pdf->Ln(5);
    $pdf->MultiCell(190,5,utf8_decode("Es por esta razón que a la fecha de la emisión de este documento, se hace constar que entre el cliente ".$Gestiones->getNombresUsuarios()." ".$Gestiones->getApellidosUsuarios()." y la sociedad financiera InversGlobal Ecuador S.A de C.V no existe adeudo alguno con lo referente al contrato referente al crédito [".$Gestiones->getNombreProductos()." ~ ".$Gestiones->getCodigoProductos()."], no así para cualquier otro tipo de adeudo por la emisión de cualquier documento financiero más."));
    $pdf->Ln(5);
    $pdf->MultiCell(190,5,utf8_decode("Sin más por el momento y a solicitud así como para los fines que al interesado convengan, se emite la presente carta en la ciudad de Ecuador, Ecuador a los ".strftime("%d días del mes de %B")." del año en curso."));
    $pdf->Ln(5);
    ...
    $pdf->Ln(25);
    $pdf->MultiCell(190,5,utf8_decode("Presidente General InversGlobal S.A de C.V"));
    $pdf->Ln(3);
    $pdf->MultiCell(190,5,utf8_decode("InversGlobal Ecuador S.A de C.V"));
    $pdf->Output();
?>