    ...
    try{
        ...
        if(MontoCredito==0 | PlazoCredito==0 | MontoCredito=="" | PlazoCredito==""){
            ...
            if(MontoCredito>=30000 && MontoCredito<=50000){// $30,000.00 - $50,0000.00 USD
                SeguroDesgravamen = 35.50;
                AhorroProgramado= 110.99;
            }else if(MontoCredito>50000 && MontoCredito<=50000){// $50,001.00 - $75,000.00 USD
               SeguroDesgravamen = 41.50;
               AhorroProgramado= 114.99;
           }else if(MontoCredito>50000 && MontoCredito<=150000){// $75,001.00 - $150,000.00 USD
               SeguroDesgravamen = 61.50;
               AhorroProgramado= 154.99;
           }else if(MontoCredito>150000 && MontoCredito<=500000){// $150,001.00 - $500,000.00 USD
               SeguroDesgravamen = 140.50;
               AhorroProgramado= 354.99;
           }else if(MontoCredito>500000){ // MAYORES A $500,000.00 USD
               SeguroDesgravamen = 540.50;
               AhorroProgramado= 954.99;
           }
             // CONVERTIR AÑOS PLAZO A MESES
             ConversorMesesPlazo = PlazoCredito*12;
             // CALCULO CUOTA MENSUAL A CANCELAR [  IVA INCLUIDO ]
             CalcularCuotaMensual = parseFloat((MontoCredito/ConversorMesesPlazo+(MontoCredito/ConversorMesesPlazo)*TasaInteres/100)+SeguroDesgravamen)*.13+parseFloat((MontoCredito/ConversorMesesPlazo+(MontoCredito/ConversorMesesPlazo)*TasaInteres/100)+SeguroDesgravamen);
        ...
        TotalDesembolso = CalculoFinanciamiento-ahorroprogramado;
...
function LimpiezaDatos(){
    // VARIABLES GLOBALES
    CalcularCuotaMensual=0; TotalDesembolso=0; CreditoSolicitado=0; MesesPlazo=0; SeguroDesgravamen=0; ahorroprogramado=0; CalculoFinanciamiento=0;
