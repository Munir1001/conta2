jQuery(".form-valide").validate({
    ...
    messages: {
        ...
        "val-dui": {
            ...
            minlength: "Su cédula debe contener 10 dígitos",
            maxlength: "No puede exceder de 10 dígitos",
        },
        "val-nit": {
            required: "Por favor ingrese su número de RUC",
            minlength: "Su Ruc debe contener 13 dígitos",
            maxlength: "No puede exceder de 13 dígitos",
        },
        "val-nombreempresa": {
        ...
        "val-telefono1": {
            minlength: "Su teléfono debe contener al menos 10 digítos",
            maxlength: "No puede exceder de 10 digítos",
        },
        "val-telefono2": {
            minlength: "Su teléfono debe contener al menos 10 digítos",
            maxlength: "No puede exceder de 10 digítos",
...
// REGISTRO NUEVOS MENSAJES -> MENSAJERIA INTERNA InversGlobal
...
// REGISTRO Y GESTION DE REFERENCIAS PERSONALES -> CLIENTES InversGlobal
...
$(document).ready(function () {
    ...
    var validator = $(".validacion-registro-cuentas-ahorros").validate({
        rules: {
    ...
    // Bloquear botón por defecto
    $('#registronuevascuentas').prop('disabled', true);

    // Validar y desbloquear el botón solo si todo es válido
    $('.validacion-registro-cuentas-ahorros input, .validacion-registro-cuentas-ahorros select').on('keyup change', function () {
        var isValid = $(".validacion-registro-cuentas-ahorros").valid();
        let monto = parseFloat($('#val-montodepositoapertura').val()) || 0;
        let tipoCuenta = $('#tipo-cuenta').val();

        let minimo = tipoCuenta === "cuenta-mi-futuro" ? 10 : 25;

        if (isValid && monto >= minimo) {
    ...
    // Mostrar mensaje según tipo de cuenta
    $('#tipo-cuenta').on('change', function () {
        ActualizarMensajeMonto();
    });

    // Recargar la página después de enviar exitosamente
    $(".validacion-registro-cuentas-ahorros").on("submit", function (e) {
        e.preventDefault(); // evitar envío por defecto
        if ($(".validacion-registro-cuentas-ahorros").valid()) {
...
// REGISTRO NUEVAS CUENTAS DE AHORRO PROGRAMADO CLIENTES
jQuery(".validacion-registro-cuentas-ahorros-programado").validate({
    rules: {
        "val-numerocuentaahorro-programado": {
            required: !0,
            minlength: 11,
            maxlength: 11,
        },
        ...
        "val-clientecuentaahorroprogramado": {
            required: !0,
        },
        "val-interesanual-programado": {
            required: true,
            number: true,
            min: 0,         // El interés no puede ser negativo
            max: 100,       // Máximo teórico del 100%
            step: 0.05      // Valida incrementos de 0.05
        }
    },
    messages: {
        "val-numerocuentaahorro-programado": {
            required: "Por favor ingrese el número único de cuenta de ahorro",
            minlength: "El número de cuenta debe contener 10 dígitos",
            maxlength: "No puede exceder de 10 dígitos",
        },
        "val-montodeahorro": {
            required: "Por favor ingrese el monto de ahorro programado",
            number: "Solo se permiten valores numéricos",
            min: "El monto mínimo es $5",
            max: "El monto máximo es $1,000,000"
        },
        "val-mesesahorro": {
            required: "Por favor ingrese los meses de su ahorro programado",
            number: "Solo se permiten valores numéricos",
            min: "El plazo mínimo es 6 meses",
            max: "El plazo máximo es 60 meses"
        },
        "val-clientecuentaahorroprogramado": {
            required: "Por favor seleccione el cliente que aperturará nueva cuenta de ahorro",
        },
        "val-interesanual-programado": {
            required: "Por favor ingrese el porcentaje de interés anual",
            number: "Solo se permiten valores numéricos",
            min: "El interés no puede ser negativo",
            max: "El interés no puede exceder el 100%",
            step: "El interés debe ser en incrementos de 0.05%"
        }
    },
...
$(document).ready(function () {
    ...
    const $submitButton = $('#registronuevascuentasahorroprogramado');
    const $form = $('#ingreso-nuevas-cuentas-ahorro-programado');

    function checkForm() {
        let allValid = true;
        $requiredFields.each(function () {
            const $field = $(this);
            // Verificación especial para campos numéricos
            ...
            if (!$field.val() || !this.checkValidity()) {
                allValid = false;
                return false;
            }
        });
    ...
    // Eventos para todos los campos
    $requiredFields.on('input change keyup paste', checkForm);
    
    // Evento adicional para campos numéricos
    $('input[type="number"]').on('blur', checkForm);
    ...
    $form.on('submit', function (e) {
        ...
        $.ajax({
            type: 'POST',
            url: $form.attr('action'), // La URL de destino del formulario
            data: $form.serialize(),
            success: function (response) {
                ...
                // Recargar la página internamente
                setTimeout(function () {
                    location.reload();
                }, 5000);
            },
            error: function () {
                alert('Hubo un error al registrar la cuenta. Intente nuevamente.');
            }
        });
    });
});
